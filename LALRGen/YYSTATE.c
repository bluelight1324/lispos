...
/* LR State Memory Management - Data Structures                                  */
 ...
 static  _TINT      newstate(ITEM **items,_TINT nitems,STATE **statep )
 {
    ...
    fprintf(stderr, "DEBUG: newstate() entered, nitems=%d, Nstates=%d\n", (int)nitems, (int)Nstates); fflush(stderr);
    ...
    fprintf(stderr, "DEBUG: newstate() calling findsym()\n"); fflush(stderr);
	if( existing =(STATE *)findsym( States, NULL ) )
	{
        fprintf(stderr, "DEBUG: newstate() findsym returned existing=%p\n", (void*)existing); fflush(stderr);
	...
	{
        fprintf(stderr, "DEBUG: newstate() creating new state, Nstates=%d, MAXSTATE=%d\n", (int)Nstates, (int)MAXSTATE); fflush(stderr);
	  if ( Nstates >= MAXSTATE )
	       error(FATAL, _T("Too many LALR(1)  states\n"));

        fprintf(stderr, "DEBUG: newstate() calling newsym()\n"); fflush(stderr);
	  if( !(state = (STATE *) newsym(sizeof(STATE)) ))
	      error( FATAL, _T("Insufficent memory for states\n") );
        fprintf(stderr, "DEBUG: newstate() newsym returned state=%p\n", (void*)state); fflush(stderr);
 ...
 */
   ...
   void    make_parse_tables()
 {
	...
	*/
    ...
    fprintf(stderr, "DEBUG: make_parse_tables() entered\n"); fflush(stderr);
    ...
    fprintf(stderr, "DEBUG: make_parse_tables() calling maketab()\n"); fflush(stderr);
	 States = maketab( MAXPROD/4,state_hash,state_cmp );
    fprintf(stderr, "DEBUG: make_parse_tables() maketab() done, States=%p\n", (void*)States); fflush(stderr);
    ...
    fprintf(stderr, "DEBUG: make_parse_tables() calling newitem()\n"); fflush(stderr);
	item = newitem( start_prod );      /* make item for start production */
	ADD( item->lookaheads, _EOI_ );      /* FOLLOW(S) = ($)     */

    fprintf(stderr, "DEBUG: make_parse_tables() calling newstate()\n"); fflush(stderr);
	newstate( &item, 1, &state );
    fprintf(stderr, "DEBUG: make_parse_tables() calling lr()\n"); fflush(stderr);
					  /* add shifts and gotos to the table */
	if( lr( state ) )
	{
        fprintf(stderr, "DEBUG: make_parse_tables() lr() returned success\n"); fflush(stderr);
	    if( Verbose )
		    printf("Adding reductions:\n");

        fprintf(stderr, "DEBUG: make_parse_tables() calling reductions()\n"); fflush(stderr);
		reductions();  /* add the reductions */
        fprintf(stderr, "DEBUG: make_parse_tables() reductions() done\n"); fflush(stderr);
		...
		{	/*
            ...
            fprintf(stderr, "DEBUG: make_parse_tables() calling print_tab(Actions)\n"); fflush(stderr);
		    print_tab( Actions,_T("__SynA"),strA, 1);
            fprintf(stderr, "DEBUG: make_parse_tables() calling print_tab(Gotos)\n"); fflush(stderr);
			print_tab( Gotos,  _T("__SynG"),strG, 1);
            fprintf(stderr, "DEBUG: make_parse_tables() print_tab done\n"); fflush(stderr);
		}
		else
		{
...
static  _TINT lr(STATE *cur_state )
{
	...
	*/
    ...
    fprintf(stderr, "DEBUG: lr() entering while loop\n"); fflush(stderr);

	while( cur_state = get_unfinished() )
	{
        fprintf(stderr, "DEBUG: lr() loop iteration, cur_state=%p, state num=%d\n", (void*)cur_state, (int)cur_state->num); fflush(stderr);
	    if( Verbose > 1)
		 ...
		 */
         ...
         fprintf(stderr, "DEBUG: lr() calling closure()\n"); fflush(stderr);
		 nclose = closure  (cur_state, closure_items, MAXCLOSE     );
         fprintf(stderr, "DEBUG: lr() closure() returned nclose=%d\n", (int)nclose); fflush(stderr);
		 nclose = kclosure (cur_state, closure_items, MAXCLOSE, nclose );
         fprintf(stderr, "DEBUG: lr() kclosure() returned nclose=%d\n", (int)nclose); fflush(stderr);
		 if( nclose )
		 {
        ...
        fprintf(stderr, "DEBUG: lr() entering inner while loop, nclose=%d\n", (int)nclose); fflush(stderr);
		while( nclose > 0 )
		{
		    first_item = p;
...
static  void  reductions()      
{
   ...
   fprintf(stderr, "DEBUG: reductions() calling ptab()\n"); fflush(stderr);
   if( !ptab( States,addreductions, NULL, 1) )
       ptab( States,addreductions, NULL, 0 );
   fprintf(stderr, "DEBUG: reductions() ptab done\n"); fflush(stderr);
}       
